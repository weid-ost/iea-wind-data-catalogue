...
flowchart TB
    ...
    subgraph T3["Tier-3 side-channel — ADR-0024/0025/0031"]
        ...
        CA[("data/cache/&lt;sha256&gt;.json<br/>COMMITTED<br/>key = content+prompt+model")]
        LLM["extract() — OpenAI-compatible POST<br/>GitHub Models via GITHUB_TOKEN"]
        PQ[("data/state/pending-extraction.json<br/>cache miss + no model = queue, continue")]
        RES["doi.resolve_or_drop<br/><i>resolves or the record is dropped and logged</i>"]
    end

    EV[("data/events/&lt;slug&gt;.jsonl<br/><b>SOURCE OF TRUTH</b><br/>append-only, append-on-change")]
    ...
    REC[("data/records/&lt;slug&gt;.json<br/>DERIVED — CKAN package dicts<br/>byte-stable")]
    VAL{"validate-ckan-compat<br/>slugs · tags · licences · string extras<br/>owner_org · groups · state"}
    RR[("data/state/last-run.json<br/>run report + HEARTBEAT")]
...
an unchanged source key writes nothing to `data/events/` but is still counted in
`data/state/last-run.json`; a validation failure is reported rather than silently
...
| append | `harvest.events.record_scrape` | `data/events/<slug>.jsonl` | append-only, append-on-change |
| annotate | `harvest.events.annotate` / `withdraw` | `data/events/<slug>.jsonl` | `local.*` only |
| replay annotations | `harvest.annotations.apply_annotations` | `data/events/<slug>.jsonl` | `data/annotations/*.yaml` → `annotated` events, idempotently; part of `run` and `materialize` |
| check pins | `harvest.annotations.check_pins` | `data/events/<slug>.jsonl` | one `pin_notice` per observed source key when a pinned page moves (§4.3) |
| resolve | `harvest.events.resolve` | nothing | folds a log into a `ResolvedRecord` |
| materialise | `harvest.materialize.materialize_all` | `data/records/<slug>.json` | byte-stable; prunes orphans |
| validate | `harvest.ckan_compat.validate_records` | nothing | prints **every** violation, not the first |
| report | `harvest.runreport.RunReport.write` | `data/state/last-run.json` | **written every run, always** |
...
replay `data/annotations/` → check pins → materialise → validate → report in one
...
`data/state/` (`merge-proposals.json`, `link-check.json`) and to the run report's
`notices`; neither ever edits `data/records/`. See [[correct-a-record]] §6 and §7.
...
4. **The cache is committed** ([[adr-0025-the-extraction-cache-is-committed]]).
   `data/cache/<sha256(content + prompt_version + model_id)>.json`. A rebuild
...
`data/state/pending-extraction.json`, and the run continues and succeeds
...
flowchart LR
    ...
    RUN --> HB["write data/state/last-run.json<br/><b>always</b>, even a total no-op"]
...
- **`data/state/last-run.json` is written on every run, including a complete no-op
...
| Tier-3 extraction, committed cache, pending queue — `harvest/extract.py` | track H | `uv run python -m harvest extract` exits 0 and prints `extract: resolved N pending extraction(s)`; the committed `data/cache/` entries replay offline |
| reconciliation, merges, link checking, `data/annotations/` replay — `harvest/dedupe.py`, `harvest/annotations.py`, `harvest/linkcheck.py`, the `dedupe` / `linkcheck` / `annotations` verbs | track I | `uv run python -m harvest dedupe` and `linkcheck` exit 0; notices appear in `data/state/last-run.json` |
| `site/` — Astro, Pagefind, the gallery, the a11y gate | track J | `make site` and `make gates` succeed |
| `.github/workflows/` | CI track | `catalogue.yml` and `ci.yml` are present; their YAML parses and every command they invoke exists in the `Makefile` or the `harvest` CLI, cross-checked command by command. **They have never executed** — a workflow cannot be run locally and neither has been pushed, so the first real run is the first proof. `catalogue.yml` is the file that will commit `data/state/last-run.json` weekly and deploy in the same job |
