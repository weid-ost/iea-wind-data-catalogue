...
(`data/annotations/doi-10-5072-zenodo-1234566.yaml`), so it sits beside the events it
...
`data/records/`. Running it a hundred times appends each annotation once. You can
...
make materialize                                 # rebuild data/records/
```
...
`data/state/last-run.json` → `notices` as `annotation_pending`.
