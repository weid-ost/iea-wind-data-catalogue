...
The inventory these implement is `data/fixtures/fixtures-catalogue.md` §"Rendering-only
...
- **`rep-NN-…`** — representative, *ordinary* records, one per source system and
  ...
  and looks like itself before `data/records/` is populated, and so the gallery shows
...
component, used only when `data/events/<slug>.jsonl` does not yet exist.
...
adapter tracks capture their own, per `data/fixtures/README.md`.
...
`data/state/last-run.json` older than 45 days, and `r-09-dead-link` is a
`data/state/link-check.json` in which one record's source link no longer responds.
None is a CKAN package, so they live one directory down, where the generic
record validation — which globs `data/fixtures/rendering/*.json` and validates each
