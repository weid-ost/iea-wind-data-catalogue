...
**Purpose:** the fixture set that drives both the harvester's tests and the `/dev/components` gallery. Every row is a real or hand-built record saved under `data/fixtures/<source>/<id>.json`, plus its raw upstream payload under `data/fixtures/<source>/raw/<id>.json` so parsing can be tested without network access.
...
**Completeness is a gate.** `tests/test_fixtures.py::TestTheCatalogueMatchesTheTree` compares the ids in these tables against `data/fixtures/*/*.json` and `data/fixtures/rendering/ui/*.json` in both directions: every row has a file, every file has a row, and no id is used twice. A row that is deliberately realised somewhere else says so in its Expected-handling cell with the words **realised as `<id>`**, and only then may it have no file of its own. The catalogue drifted seventeen fixtures and one duplicate id before that gate existed (fixture-compliance-05, -06).

**Invented fixtures declare themselves**, in the expectation *and* in the raw payload, and they never wear a live identifier: synthetic DOIs sit on the reserved DataCite test prefix `10.5072`, which does not resolve. See `data/fixtures/README.md`.
...
| `cr-08-reference-list` | A journal article with 25 reference entries, 19 of them carrying a DOI | The citation edges the scope rule runs on (ADR-0043) | `reference` is requested — it was deliberately excluded as bulky until the catalogue acquired a rule that needs it — and `map()` keeps the 19 DOIs and discards everything else in each entry (`key`, `unstructured`, `doi-asserted-by`, and the cited work's own metadata). A citation edge needs an identifier and nothing else |
...
| `x-13-out-of-scope-topic-sweep` | A repository found only through the generic `wind-energy` GitHub topic | The scope rule's exclusion path (ADR-0043) | `inclusion_basis: none`. No IEA Wind attribution, no citation link either way, so it is **not listed** — and not deleted: the page and the URL keep resolving, and the record states why it is excluded. The decision is re-made on every rebuild, so a later citation puts it back with no intervention |
| `x-14-in-scope-by-citation` | The same generic route, but the repository's README cites a Zenodo deposit the catalogue holds | The scope rule's second limb | `inclusion_basis: cites`, with the cited record named in the evidence. One stated edge fills both directions. Limb 1 is tried first and is stronger, which is why the reference turbine beside it reports `direct` through its IEA Wind community rather than `cited-by` |
...
| `x-21-dedupe-fuzzy-proposal` | The same work in Zenodo and an institutional repository with no shared DOI | Realises `dc-08` | A merge **proposal** written to `data/state/merge-proposals.json` and never applied. Applying it twice changes nothing |
...
Representative, not pathological: one per source system and one per interesting-but-not-broken state, so the gallery shows normal next to awkward and the site has something to render before `data/records/` is populated.
