...
curl -sS -H 'Accept: application/json' \
  ...
  | python -m json.tool > data/fixtures/zenodo/raw/zen-01-canonical.json
```
...
* `"degradation"` — a source that disables itself. There is nothing upstream
  ...
  (`wdh-07`).
* `"ui_state"` — not a record at all: a `data/state/*.json` payload the site renders
  (`r-07`, `r-08`, `r-09`). These live in `data/fixtures/rendering/ui/` so the
...
```jsonc
{
  ...
  // optional — the YAML a curator would have written in data/annotations/
