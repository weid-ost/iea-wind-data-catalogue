...
``data/events/<slug>.jsonl``, one file per identity, append-only, **append-on-change
...
``data/state/last-run.json`` instead (ADR-0026).

``data/records/`` is a materialised view of this directory and can be deleted and
regenerated at any time. This directory cannot.

Note on filenames: ADR-0037 speaks of ``data/events/<identity-key>.jsonl``, but an
...
slug used for ``data/records/<slug>.json``, the CKAN ``package.name`` and the site
...
#: only in ``data/state/merge-proposals.json`` is not durable: that file is
#: rewritten wholesale on every pass, while ADR-0037 makes ``data/events/`` the
...
def event_path(identity_key: str, events_dir: Path | None = None) -> Path:
    """``data/events/<slug>.jsonl`` for an identity key.
...
def append_event(identity_key: str, event: Event, events_dir: Path | None = None) -> Path:
    ...
    The only write path into ``data/events/``. Never rewrites or reorders a line.
...
#: stop ``data/state/last-run.json`` being written (CONTRACT rule 5, ADR-0029).
...
def replay(
    ...
    This is the function ``data/records/*.json`` is generated by. Import cycle is
